        ...
        <div class="tab-menu">
            <div class="tab-bar">
              <span class="tab-group">
                <span class="tab-item" v-for="(item, index) in tabsData" :index = "index" v-show="item.access" v-bind:class="item.selected ? 'active':''" @click="tabChange(index)">{{item.name}}</span>
              </span>
            </div>
        </div>
      </div>
  ...
  methods: {
    ...
    getVoucher () {
      ...
      commonDetail.getVoucher.remote({id: this.voucherId}, (result) => {
        ...
        this.$bus.$emit('voucher-operate', this.detail)
      })
    },
    ...
    showTagMenu () {
      ...
      var module = (position > -1) ? hash.substr(0, position) : hash
