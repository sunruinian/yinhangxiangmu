<template>
  <div class="content-area" >
    ...
    <el-tabs v-model="tab.activeName" @tab-click="tabHandleClick">
      <el-tab-pane label="应收凭证" name="first" v-if="indexAccess"></el-tab-pane>
      <el-tab-pane label="应付凭证" name="second" v-if="recordAccess"></el-tab-pane>
    </el-tabs>
...
export default {
  ...
  mounted () {
    ...
    this.indexAccess = accessControl.checkAccess('/voucher/query/index')
    this.recordAccess = accessControl.checkAccess('/voucher/query/record')
  },
  methods: {
    tabHandleClick (item) {
      let url = ''
      if (item.name === 'first') {
        url = '#/voucher/query/index'
      } else if (item.name === 'second') {
        url = '#/voucher/query/record'
      }
