...
import voucherService from 'common/api/voucher/common.js'
  ...
  methods: {
    ...
    dialogToggle () {
      const info = {
        ...
        payAmount: this.formField.transformAmount,
        payAmount2: this.formField.transformAmountBig,
      ...
      this.$refs.formField.validate((valid) => {
        ...
        if (valid) {
          ...
          detail.createCompany = {
            ...
            subAccount: {
              bankName: this.currentCompany.subAccount.issuer.name,
              no: this.currentCompany.subAccount.no
            }
          }
          detail.company = {
    ...
    redirect () {
      ...
      supplierApi.transferVoucher.remote(data, (result) => {
        if (result.data) {
          alert('转让成功')
