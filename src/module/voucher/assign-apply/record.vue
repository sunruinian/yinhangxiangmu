<template>
  <div class="content-body" id="assignRecord">
            ...
            <el-col :span='24'>
              <el-form :inline="true" :model="searchData" class="stand-form " label-width="100px">
                  <fieldset>
                    ...
                    <el-form-item label="接受方" label-width="65px">
                      <el-input v-model="searchData.companyQstr" placeholder="请输入接受方"></el-input>
                    </el-form-item>
                    <el-form-item label="申请日期" class="last-item" label-width="80px">
                      <el-date-picker
                        v-model="payDate"
                        type="daterange"
                        :editable=false
...
export default merge({
  ...
  data () {
    return {
      title: '申请记录',
      expire: [],
      statusOptions: status.transferApplyStatus,
      searchData: {
        no: null,
        companyQstr: null,
        cashTime: null,
  ...
  mounted () {
    this.getGrid()
    this.detailAccess = accessControl.checkAccess('/voucher/assign-apply/record/detail')
  },
  methods: {
    // 查询凭证
    getGrid () {
      ...
      supplierApi.getTransferApplyRecord.remote(params, (result) => {
    ...
    toInfo (id) {
      location.href = '/#/voucher/assign-apply/record/detail?id=' + id
    },
    expireFun () {
