<template>
  <div class="content-area" >
    ...
    <el-tabs v-model="tab.activeName" @tab-click="tabHandleClick">
      <el-tab-pane label="待审批转让" name="first" v-if="indexAccess"></el-tab-pane>
...
export default {
  ...
  mounted () {
    ...
    this.indexAccess = accessControl.checkAccess('/voucher/assign-approve/index')
    this.recordAccess = accessControl.checkAccess('/voucher/assign-approve/record')
  },
  methods: {
    tabHandleClick (item) {
      let url = ''
      if (item.name === 'first') {
        url = '#/voucher/assign-approve/index'
      } else if (item.name === 'second') {
        url = '#/voucher/assign-approve/record'
      }
