...
<template>
  <div class="content-area" id="created">
    <div v-title='title'>
      {{title}}
    </div>
      <div class="group">
      ...
      <div class="content-body" id='payapplyInfo'>
        ...
        <!-- <div class="font1" style="width:942px;margin:0px auto;">
          <span class="require">＊</span>申请通过后该笔付款资金将从您的账户转移至接受方（收款方）账户。
        </div> -->
        ...
        value: [
          { required: true, message: '必须填写', trigger: 'change' },
          {
            validator: (rule, value, callback) => {
              ...
              const reg = /^\d+(\.\d{1,2})?$/
              if (value <= 0) {
                callback(new Error('金额为非零正数'))
              }
              if (parseFloat(value) > 999999999.99) {
                callback(new Error('金额过大！'))
              }
              if (!reg.test(value)) {
                callback(new Error('金额为非零正数，保留小数点后两位'))
              }
  ...
  methods: {
    redirect () {
      this.$refs.ruleForm.validate((valid) => {
        ...
        if (valid) {
          let params = {
            applyReason: this.ruleForm.desc,
            vouId: this.vouId,
    ...
    getVoucher () {
      ...
      commonDetail.getVoucher.remote({id: this.id}, (result) => {
        this.detail = result.data
        this.detail.value = result.data.total
        this.detail.paymentLetter = result.data.paymentLetter.no
...
<style>
#payapplyInfo .el-form-item__error {
  width: 225px;
  top: 38px;
}
</style>
