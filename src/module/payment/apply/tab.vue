<template>
  <div class="content-area" >
    ...
    <el-tabs v-model="tab.activeName" @tab-click="tabHandleClick">
      <el-tab-pane label="付款申请" name="first" v-if="applyAccess"></el-tab-pane>
...
export default {
  ...
  mounted () {
    ...
    // }
    this.applyAccess = accessControl.checkAccess('/payment/apply/index')
    this.recordAccess = accessControl.checkAccess('/payment/apply/record')
  },
  methods: {
    tabHandleClick (item) {
      let url = ''
      if (item.name === 'first') {
        url = '#/payment/apply/index'
      } else if (item.name === 'second') {
        url = '#/payment/apply/record'
      }
