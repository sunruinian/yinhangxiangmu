...
export default {
  ...
  mounted () {
    ...
    if (role === '2') {
      ...
      commonDetail.getCurrentCompany.remote({}, (info) => {
        ...
        const inte = setInterval(() => {
          if (this.times === 1) {
            this.$bus.$emit('on-step', config.authStep) // 设置信息
    ...
    // alert(role)
    if (role === '1') {
      this.msg.tit = '已完成实名认证！'
      // this.msg.content = '审核通过后，系统将自动添加您的授信及结算户信息。'
      this.showTimeRun = false
      commonMember.userAuthStep.remote({}, (info) => {
        if (parseInt(info.data) === config.authStep) {
          cookieTools.setCookie('step-register', config.authStep)
      ...
      commonMember.userAuthStep.remote({}, (info) => {
        if (parseInt(info.data) === 4) {
          cookieTools.setCookie('step-register', config.authStep)
          location.href = '#/customer/f-core-enterprise'
        }
      })
    }
  },
