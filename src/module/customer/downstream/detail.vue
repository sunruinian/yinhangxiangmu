                  ...
                  <table class="table-lr">
                    <tbody>
                      <tr>
                          ...
                          <td class="r" width="25%">{{detail.upstreamName}}</td>
                          <td class="l" width="25%">供应商ERP编号</td>
                          <td class="r" width="25%">{{detail.upstreamRemark}}</td>
                      </tr>
                      <tr v-show = "show">
                      ...
                      <tr>
                          ...
                          <td class="r" width="25%">{{detail.updateTime}}</td>
                          <td class="l" width="25%">邀请码</td>
                          <td class="r" width="25%">{{detail.invite}}</td>
                      </tr>
                      <tr>
                          <td class="l" width="25%">供应商编号</td>
                          <td class="r" width="25%">{{detail.upstreamId | sixEnterpriseCode}}</td>
                          <td class="l" width="25%">区块链账户</td>
                          <td class="r" width="25%">{{detail.upstreamBlockchainAddress}}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                  ...
                  <div class="group one-row">
                      <div class="span3 offset1 ">
                          <div class="image-content">
                              ...
                              <img alt="" :src="'/service/file/upstream/download/' + realName.busy + '/' + realName.id">
                          </div>
                      </div>
                      <div class="span3">
                          <div class="image-content">
                              <span class="title">代表人身份证</span>
                              <img alt="" :src="'/service/file/upstream/download/' + realName.detail.corpCardFileId + '/' + realName.id">
                          </div>
                      </div>

                      <div class="span3" v-if="realName.detail.clientCardFileId !== 0">
                          <div class="image-content">
                              <span class="title">委托人身份证</span>
                              <img alt="" :src="'/service/file/upstream/download/' + realName.detail.clientCardFileId + '/' + realName.id">
                          </div>
                      </div>

                      <div class="span3" v-if="realName.detail.otherFileId !== 0">
                          <div class="image-content">
                              <span class="title">其他材料</span>
                              <img alt="" :src="'/service/file/upstream/download/' + realName.detail.otherFileId + '/' + realName.id">
                          </div>
                      </div>
                  </div>
                </div>
  ...
  methods: {
    ...
    getVoucher () {
      ...
      if (this.tabsData[0].access) {
        coreDown.downstreamInfo.remote(this.id, (data) => {
          if (data.data.contact === '') {
            this.show = false
          }
          this.detail = data.data || this.detai
          this.detail.contactType = this.detail.contactType === 0 ? '短信通知' : (this.detail.contactType === 1 ? '邮件通知' : '')
          this.detail.upstreamId = this.detail.upstreamId
          this.detail.updateTime = dateTools.dateFormater(this.detail.updateTime, 'YYYY.MM.DD HH:mm:ss')
        })
      }
      // 实名信息
      if (this.tabsData[1].access) {
        coreDown.downstreamAuth.remote(this.sId, (data) => {
          if (data.data.certificateList) {
            this.tabsData[1].data = data.data || null
            ...
            for (let i = 0; i < status.institutionType.length; i++) {
              if (status.institutionType[i].value === this.realName.detail.type) {
                this.realName.type = status.institutionType[i].label
              }
            }
            ...
            Array.map(this.realName.fileList, (item, i, arr) => {
              this.file[item.fileType] = tools.getUrl() + '/service/file/upstream/download/' + '/' + item.fileId + '/' + this.realName.id
            })
          }
        })
      }
