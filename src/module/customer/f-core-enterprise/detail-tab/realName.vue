<template>
  <div class="content-body info-manage pr20 pb40 pl20">
     <div v-title='title' v-if='tabShow'>{{title}}</div>
      <div v-if='realInfoShow'>
              ...
              <table class="table-lr">
                <tbody>
                  ...
                  <tr>
                      ...
                      <td class="l" width="25%">统一社会信用代码</td>
        ...
        <div class="item-group">
            ...
            <div class="item-cnt">
                <el-row>
                  ...
                  <el-col v-if="realName.detail.clientCardFileId !== 0" :span="6">
                    <div class="image-content">
                      <span class="title">委托人身份证</span>
                      <div class='imgContainer'>
                        <img alt="" :src="'/service/file/image/download/' + realName.detail.clientCardFileId">
                        <div class="imgPreview" @click.stop='imgPreview("/service/file/image/download/" + realName.detail.clientCardFileId)'>
                          预览
                        </div>
                      </div>
                    </div>
                  ...
                  <el-col v-if="realName.detail.otherFileId !== 0" :span="6">
                    <div class="image-content">
                      <span class="title">其他材料</span>
                      <div class='imgContainer'>
                        <img alt="" :src="'/service/file/image/download/' + realName.detail.otherFileId">
                        <div class="imgPreview" @click.stop='imgPreview("/service/file/image/download/" + realName.detail.otherFileId)'>
                          预览
                        </div>
                      </div>
                    </div>
      ...
      <div v-if='!realInfoShow' class="noDataMsg"><i class="el-icon-warning"></i> 尚未进行实名认证</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    init () {
      // 实名信息
      coreDown.downstreamAuth.remote(this.id, (data) => {
        if (data.data === null) {
