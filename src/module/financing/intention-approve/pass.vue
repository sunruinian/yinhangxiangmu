...
export default merge(table, {
  ...
  mounted () {
    // 搜索框数据
    this.$bus.$on('finance-data-two', (data) => {
  ...
  beforeDestroy () {
    // 销毁bus监听事件，避免多次触发
    this.$bus.$off('finance-data-two')
  },
  methods: {
    getGrid () {
      bankService.queryPassed.remote(this.searchData, (list) => {
    ...
    toInfo (id) {
      location.href = '#/financing/intention-approve/pass-detail?id=' + id
    }
  }
})
</script>
