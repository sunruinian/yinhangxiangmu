                      ...
                      <el-row>
                        <el-col>
                          <div class="item-cnt">
                              <!--<el-row>
                                  <el-col :span="20">
                                      <el-form-item label="选择融资方式" prop="financingType" class="is-required">
                                          <el-radio-group v-model="params.financingType">
                                            <el-radio label="HYPOTHECATION">质押融资</el-radio>
                                            <el-radio label="FACTOR">保理融资</el-radio>
                                          </el-radio-group>
                                          <div v-show='message.financingType' class="el-form-item__error" style="margin-top: -9px">请选择融资方式</div>
                                      </el-form-item>
                                  </el-col>
                              </el-row>-->
                                  ...
                                  <el-row>
                                    <el-col>
                                      ...
                                      <div class="item-cnt">
                                        <el-row >
                                          <el-col :span="8">
                                            <el-form-item label="本次融资金额"  prop='financingAmount' label-width="108px">
                                                <el-input
                                                  placeholder="请输入融资金额"
                                          ...
                                          <el-col :span="8">
                                            <el-form-item label="融资期限" prop='financingLimit' label-width="80px">
                                                <el-input
                                                  placeholder="请输入融资期限"
                                                  v-model="params.financingLimit">
                              ...
                              <div class="pl30 pr30">
                                <div class="button-group mt30 mb30">
                                    <el-button class="confirm mr20" type="info" @click="goback">取消</el-button>
...
export default {
  ...
  data () {
    return {
      ...
      params: {
        ...
        financingType: 'HYPOTHECATION',
        financingLimit: null,
        voucherId: null
      },
      hasData1: 0,
      hasData2: 0,
      financingAmountBig: null,
      notSet: '-',
      voucher: {
        originCreateCompany: {
          name: null
        ...
        financingType: [
          {
            validator: (rule, value, callback) => {
              this.message.financingType = !value ? 'true' : false
              callback()
            },
            trigger: 'change' }
        ],
  ...
  methods: {
    getGrid () {
      supplierFinancing.voucherDetail.remote(this.params.voucherId, (list) => {
        if (list.success) {
          this.hasData1 = 1
          this.voucher = list.data || []
          this.params.financingAmount = moneyTools.moneyShow(this.voucher.restValue)
          this.financingAmountBig = smallToBig(this.voucher.restValue)
        } else {
          this.hasData1 = 2
        }
      })
      // 贷款方列表
      supplierFinancing.bankQuery.remote((list) => {
    ...
    twoDigit () {
      this.params.financingAmount = moneyTools.delcommafy(this.params.financingAmount)
      this.$refs.ruleForm.validateField('financingAmount', valid => {
        if (!valid) {
          // 处理金额大写
          this.financingAmountBig = smallToBig(this.params.financingAmount)
        }
      })
      // 保留两位小数 + 千分符
      this.params.financingAmount = moneyTools.moneyShow(this.params.financingAmount)
    },
    goback () {
      if (confirm('确定取消融资吗？')) {
        history.go(-1)
    ...
    submit () {
      ...
      // 表单校验
      switch (true) {
        // case !this.params.financingType: this.message.financingType = true
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid && !result) {
          let param = {
            ...
            financingAmount: moneyTools.delcommafy(this.params.financingAmount),
            financingType: this.params.financingType,
            financingLimit: this.params.financingLimit,
            voucherId: this.params.voucherId
          }
          supplierFinancing.financingCreate.remote(param, (list) => {
            if (list.success) {
              if (list.data) {
