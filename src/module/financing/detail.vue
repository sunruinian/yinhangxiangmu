...
export default {
  ...
  mounted () {
    ...
    var detailStart = hash.indexOf('/voucher')
    var module = hash.substring(11, detailStart)
    this.tabsData[0].access = accessControl.checkAccess('/financing/' + module + '/voucher/detail/base') || accessControl.checkAccess('/payment/' + module + '/detail/base')
    this.tabsData[0].selected = this.tabsData[0].access // 激活还是不激活与权限一致，无权限：不激活；有权限：激活
    this.tabsData[1].access = accessControl.checkAccess('/financing/' + module + '/voucher/detail/trade') || accessControl.checkAccess('/payment/' + module + '/detail/trade')
    this.tabsData[1].selected = this.tabsData[1].access ? !this.tabsData[0].selected : this.tabsData[1].access // 无权限：不激活；有权限：第1个tab未激活才激活
    this.tabsData[2].access = 0 // accessControl.checkAccess('/financing/' + module + '/voucher/detail/downstream') || accessControl.checkAccess('/payment/' + module + '/detail/downstream')
    this.tabsData[2].selected = 0 // this.tabsData[2].access ? !this.tabsData[0].selected && !this.tabsData[1].selected : this.tabsData[2].access // 无权限：不激活；有权限：第1个和第2个tab未激活才激活
    this.tabsData[3].access = 0 // accessControl.checkAccess('/financing/' + module + '/voucher/detail/upstream') || accessControl.checkAccess('/payment/' + module + '/detail/upstream')
    this.tabsData[3].selected = 0 // this.tabsData[3].access ? !this.tabsData[0].selected && !this.tabsData[1].selected && !this.tabsData[2].selected : this.tabsData[3].access // 无权限：不激活；有权限：第1个和第2个tab未激活才激活
    this.tabsData[4].access = accessControl.checkAccess('/financing/' + module + '/voucher/detail/paymentApply') || accessControl.checkAccess('/payment/' + module + '/detail/paymentApply')
    ...
    this.tabsData[5].access = accessControl.checkAccess('/financing/' + module + '/voucher/detail/operate') || accessControl.checkAccess('/payment/' + module + '/detail/operate')
