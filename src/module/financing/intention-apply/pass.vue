...
export default merge(table, {
  ...
  data () {
    return {
      ...
      tableData: [],
      appliedDetailAccess: false
    }
  },
  mounted () {
    // 搜索框数据
    this.$bus.$on('search-data-two', (data) => {
    ...
    this.getGrid()
    this.appliedDetailAccess = accessControl.checkAccess('/financing/intention-apply/pass-detail')
  },
  beforeDestroy () {
    // 销毁bus监听事件，避免多次触发
    this.$bus.$off('search-data-two')
  },
  methods: {
    getGrid () {
      supplierService.queryPassed.remote(this.searchData, (list) => {
    ...
    toInfo (id) {
      location.href = '#/financing/intention-apply/pass-detail?id=' + id
    }
  }
})
</script>
